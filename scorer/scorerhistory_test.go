...
func TestScorerMap_SkipIteration(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			ls:      ntpdb.LogScore{ServerID: 1, Score: 20.0, Ts: now.Add(-2 * time.Minute)},
			want:    false,
		},
		{
			name: "no prior compute does not skip",
			ls:   ntpdb.LogScore{ServerID: 1, Score: 20.0, Ts: now.Add(-30 * time.Minute)},
			want: false,
		},
		{
			name:    "mild lag, close score, within 5 min: skip",
			durable: map[int]*lastUpdate{1: {ts: now.Add(-32 * time.Minute), score: 20.0}},
			ls:      ntpdb.LogScore{ServerID: 1, Score: 20.1, Ts: now.Add(-30 * time.Minute)},
			want:    true,
		},
		{
		...
		},
		{
			...
			durable: map[int]*lastUpdate{1: {ts: now.Add(-32 * time.Minute), score: 20.0}},
			ls:      ntpdb.LogScore{ServerID: 1, Score: 25.0, Ts: now.Add(-30 * time.Minute)},
			want:    false,
		},
		{
			name:    "heavy lag, close score within 18 min window: skip",
			durable: map[int]*lastUpdate{1: {ts: now.Add(-135 * time.Minute), score: 20.0}},
			ls:      ntpdb.LogScore{ServerID: 1, Score: 20.0, Ts: now.Add(-120 * time.Minute)},
			want:    true,
		},
		{
